...
import { mockPedidos, mockClientes, mockProdutos } from '@/data/mockData';
...
export const PedidosView = () => {
  ...
  const statusCounts = mockPedidos.reduce((acc, pedido) => {
    acc[pedido.status] = (acc[pedido.status] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  const totalValue = mockPedidos.reduce((sum, pedido) => sum + pedido.valorTotal, 0);
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-2xl font-bold">{mockPedidos.length}</p>
