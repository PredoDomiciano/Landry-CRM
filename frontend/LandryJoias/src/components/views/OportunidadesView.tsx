...
import { mockOportunidades, mockClientes } from '@/data/mockData';
...
export const OportunidadesView = () => {
  ...
  const filteredOportunidades = mockOportunidades.filter(oportunidade => {
  ...
  const statusCounts = mockOportunidades.reduce((acc, op) => {
    acc[op.status] = (acc[op.status] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  const totalValue = mockOportunidades.reduce((sum, op) => sum + op.valor_estimado, 0);
